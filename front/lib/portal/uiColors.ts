/**
 * Ported from biogenome-client `useApplicationSettings.ts` — generates extended color variables.
 * Extended with `accent` for the Next.js theme layer.
 */
type RGB = { r: number; g: number; b: number }

export function normalizeUiColors(ui: Record<string, unknown>): Record<string, unknown> {
   const baseVariables = (ui.colors as { variables?: Record<string, string> })?.variables ?? {}
   ...
   const rawAccent = normalizeHex(baseVariables.accent || rawSecondary)
   const primary = desaturate(rawPrimary, 0.72)
   const secondary = desaturate(rawSecondary, 0.72)
   const accent = desaturate(rawAccent, 0.72)
   const nav = secondary
...
function buildStatusPalette(secondaryHex: string): Record<string, string> {
   ...
   const l = 0.46
   return {
      status0: rgbToHex(hslToRgb(8, s, l)),
      status1: rgbToHex(hslToRgb(25, s, l + 0.02)),
      status2: rgbToHex(hslToRgb(38, s, l + 0.03)),
      status3: rgbToHex(hslToRgb(55, s, l + 0.02)),
      status4: rgbToHex(hslToRgb(85, s - 0.05, l)),
      status5: rgbToHex(hslToRgb(115, s - 0.1, l - 0.01)),
      status6: rgbToHex(hslToRgb(148, s - 0.15, l - 0.02)),
   }
}
