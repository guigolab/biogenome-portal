...
function setPath(taxid: string) {
    ...
    if (node) {
        ...
        toast.add({ severity: 'warn', summary: 'Warning', detail: `${itemStore.parentTaxon?.name} is not under ${taxonomyStore.treeData?.name}`, life: 3000 });
...
function createTree(data: any) {
    ...
    const mytree = new hyt.Hypertree(
        { parent: hypertree.value as any },
        {
            ...
            interaction: {
                onNodeClick: (n, m, l) => {
                    itemStore.parentTaxon = {...n.data.data}
