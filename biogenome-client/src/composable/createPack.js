...
export function createPack(pack, wrapper, root, domains) {
   packRoot = d3.pack().size([width, height]).padding(5)(
      root.sum((d) => (d.children.length === 0 ? 1 : 0)).sort((a, b) => b.value - a.value),
   )
   ...
   node
      .append('title')
      .text((d) =>
         !d.data.leaves ? `${d.data.name}` : `${d.data.name} (${d.data.rank})\nRelated Organisms: ${d.data.leaves}`,
      )
