   ...
   <div>
      <div class="map-container">
         ...
         <MapStatsDropdown
            ...
            :filtered-counts="filteredCounts"
            :polygon="polygon"
            :taxid="currentTaxon?.taxid"
            @close="showStatsDropdown = false"
         />
      </div>
...
<script setup lang="ts">
   ...
   const filteredCounts = computed(() => {
      return Object.entries(counts.value)
         .filter(([key, value]) => models.value.includes(key) && value > 0)
         .map(([key, value]) => ({ key, value }))
   })

   watch(
      () => [currentTaxon.value, polygon.value],
      async () => {
   ...
   function handleRoute(route: { name: string; params: { model: string; id: string } }) {
      itemStore.model = route.params.model as DataModels
      itemStore.selectItem(route.params.id)
