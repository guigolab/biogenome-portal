<template>
   ...
   <div class="row">
      <div class="flex lg12 md12 sm12 xs12">
         <VaCard>
            <VaCardContent>
               <h2 class="va-h6">Import Form</h2>
               <p class="va-text-secondary">
                  Type a valid accession to insert into the database, in case of experiment or assembly, the related
                  biosample will also be imported
               </p>
            </VaCardContent>
            <VaCardContent>
               <div class="row">
                  ...
                  <div class="flex lg6 md6 sm12 xs12">
                     <VaInput v-model="input" placeholder="Type a INSDC accession"> </VaInput>
                  </div>
               </div>
            </VaCardContent>
            <VaCardActions>
...
<script setup lang="ts">
   ...
   const props = defineProps<{
      importModel?: 'biosamples' | 'assemblies' | 'reads'
   }>()

   const { init } = useToast()

   const isLoading = ref(false)
   const options = ['biosamples', 'assemblies', 'reads']
