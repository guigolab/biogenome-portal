<template>
            ...
            <VaCardContent>
               <VaDataTable hoverable :items="items" :columns="currentConfig.columns">
                  ...
                  <template #cell(delete)="{ rowData }">
                     <VaChip size="small" color="secondary" v-if="isOrganisms && !isAdmin && rowData.pending_deletion">
                        Pending Deletion
                     </VaChip>
                     <VaChip
                        v-else
                        ...
                        color="danger"
                        icon="delete"
                        >{{ isOrganisms && !isAdmin ? 'Send delete request' : 'Delete' }}</VaChip
                     >
                  </template>
                  <template v-if="currentConfig.editRoute" #cell(edit)="{ rowData }">
                     <VaChip
                        color="secondary"
                        @click="router.push(currentConfig.editRoute(rowData))"
                        size="small"
                        icon="edit"
                        >Edit
                     </VaChip>
   ...
   <VaModal v-model="showModal" hide-default-actions>
      ...
      <div class="p-10">
         ...
         <p v-else>All its related data will be also deleted!!</p>
      </div>
      <template #footer>
...
<script setup lang="ts">
   ...
   const buttonAction = computed(() => {
      ...
      } else if (props.model === 'reads') {
         return { label: 'Import read runs', route: { name: 'insdc-form', params: { importModel: 'reads' } } }
