<template>
   <div>
      <div class="row">
         <div class="flex">
            <Header title="Organism Access Overview" description="List of assigned and unassigned organisms"> </Header>
         </div>
      </div>
      <div class="row align-center justify-space-between">
         <div class="flex">
            <div class="row">
               <div class="flex">
                  <VaButtonToggle preset="secondary" border-color="primary" v-model="toggle" :options="toggleOptions">
                  </VaButtonToggle>
               </div>
               <div class="flex">
                  <VaInput
                     ...
                     placeholder="Filter by scientific name or taxid"
                  ></VaInput>
               </div>
               ...
               <div class="layout va-gutter-3">
                  <div style="max-width: 200px" class="row">
                     <div class="flex lg12 md12 sm12 xs12">
                        <VaCheckbox color="primary" label="Apply current filters" v-model="applyFilters"> </VaCheckbox>
                     </div>
                     <div class="flex lg12 md12 sm12 xs12">
            ...
            <VaCard>
                  ...
                  <VaDataTable :items="organisms" :columns="currentCols" :loading="isLoading">
                     <template #cell(assigned_users)="{ rowData }">
                        <VaChip
                           icon="fa-edit"
                           size="small"
                           :to="{ name: 'update-user', params: { name } }"
                           style="margin: 0 3px 0 3px"
                           v-for="name in rowData.assigned_users"
                        >
                           {{ name }}
                        </VaChip>
                     </template>
               ...
               <VaCardContent>
                  <div class="row justify-space-between align-center">
                     <div class="flex">Results: {{ total }}</div>
                     <div class="flex">
                        <VaPagination
                           color="textPrimary"
                           v-model="offset"
...
<script setup lang="ts">
   ...
   const currentCols = computed(() =>
      toggle.value === 'assigned' ? columns : columns.filter((c) => c !== 'assigned_users'),
   )

   const filter = ref('')
   const users = ref<Record<string, any>[]>([])
   const callback = computed(() =>
      toggle.value === 'assigned' ? OrganismService.getOrganismsWithUsers : OrganismService.getUnassignedOrganisms,
   )
   ...
   const query = computed(() =>
      toggle.value === 'assigned'
         ? { name__in: usersQuery.value, filter: filter.value, ...pagination.value }
         : { filter: filter.value, ...pagination.value },
   )
