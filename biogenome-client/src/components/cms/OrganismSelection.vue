<template>
   <div class="row">
      <div class="flex lg12 md12 sm12 xs12">
         <VaInput
            v-model="taxonFilter"
            placeholder="Type a valid scientific name or a taxid"
            clearable
      ...
      <div v-if="goToUpdateForm && selectedTaxon" class="flex lg12 md12 sm12 xs12">
         <VaCard stripe-color="warning" stripe>
            ...
            <VaCardActions>
               <VaButton preset="primary" :to="{ name: 'update-organism', params: { taxid: selectedTaxon.taxId } }">
                  Update organism</VaButton
               >
            </VaCardActions>
         </VaCard>
      </div>
...
<script setup lang="ts">
   ...
   async function selectTaxon(taxon: { taxId: string; scientificName: string; lineage?: string }) {
      ...
      if (!isAdmin.value && !userSpecies.value.includes(taxon.taxId)) {
         ...
         init({
            message: `The organism ${taxon.scientificName} already exists in the database, but you haven't the rights to modify its data`,
            color: 'danger',
         })
         return
      }
      ...
      try {
         ...
         if (data && data.taxid === taxon.taxId) {
            if (isAdmin.value || userSpecies.value.includes(taxon.taxId)) {
               ...
               init({
                  message: `The organism ${taxon.scientificName} already exists in the database, you can update its data by clicking on the button: Update organism`,
                  color: 'warning',
               })
            } else {
               unauthorized.value = true
