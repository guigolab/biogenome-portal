   ...
   <div class="row">
      <div class="flex lg12 md12 sm12 xs12">
         <VaCard outlined square>
            <VaCardContent>
               <h3 class="va-h5">Coordinates Validation</h3>
               <p class="va-text-secondary">
                  Select the fields related to the sample coordinates if presents, if you update the fields value you
                  must create another map overview to see the changes
               </p>
            </VaCardContent>
            <VaCardContent>
               <div class="row align-end">
                  ...
                  <div class="flex">
                     <VaButton :disabled="isDisabled" @click="createMap">See in Map</VaButton>
                  </div>
               </div>
               <VaCard v-for="(loc, idx) in locations" square outlined>
                  <VaCardContent>
                     <div class="row align-center">
                        <div class="flex"><span class="va-text-bold">Latitude:</span> {{ loc.coordinates[1] }}</div>
                        <div class="flex"><span class="va-text-bold">Longitude:</span> {{ loc.coordinates[0] }}</div>
                        <div class="flex">
                           <VaButton @click="locations.splice(idx, 1)" color="danger">Delete view </VaButton>
                        </div>
                     </div>
                  </VaCardContent>
                  <VaCardContent>
