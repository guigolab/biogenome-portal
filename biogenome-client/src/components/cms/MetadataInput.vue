<template>
   <VaCard>
      <VaCardContent>
         ...
         <p class="va-text-secondary">Add custom metadata</p>
      </VaCardContent>
      <VaCardContent>
         <div v-for="(mt, index) in organismStore.metadataList" :key="index" class="row align-center">
            <div class="flex">
               <VaInput
                  :error="organismStore.metadataList.filter((m) => m.key === mt.key).length > 1"
                  v-model="mt.key"
         ...
         <div class="row">
            <div class="flex lg12 md12 sm12 xs12">
               <VaButton icon="add" @click="organismStore.metadataList.push({ key: '', value: '' })"
                  >Add new attribute</VaButton
               >
            </div>
         </div>
      </VaCardContent>
