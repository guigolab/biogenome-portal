      ...
      <VaCardContent>
         ...
         <div class="row">
            <div class="flex lg12 md12 sm12 xs12">
               <VaButton icon="add" @click="organismStore.images.push({ value: '' })">Add new image</VaButton>
            </div>
         </div>
      </VaCardContent>
...
<script setup lang="ts">
   ...
   function isImageUrl(url: string): Promise<boolean> {
      return new Promise((resolve) => {
         ...
         const img = new Image()
         img.onload = () => {
            imageOkCache.set(url, true)
            resolve(true)
         }
         img.onerror = () => {
            imageOkCache.set(url, false)
            resolve(false)
         }
         img.src = url
