         ...
         <div v-if="openedModel === model.key" class="stats-accordion-content">
            <div class="stats-actions-row">
               <VaButton
                  ...
                  @click.stop="download('jsonl', model.key)"
               >
                  JSONL
               </VaButton>
               <VaButton
                  ...
                  preset="secondary"
                  icon="fa-file-arrow-down"
                  @click.stop="download('tsv', model.key)"
               >
                  TSV
               </VaButton>
            </div>
               ...
               <div class="stats-infinite-scroll-wrapper">
                  ...
                  >
                     <div class="custom-sample-list">
                        <div
                           v-for="row in relatedRecords"
                           :key="row.id || row.accession || row.taxid || row.local_id || row.name"
                           class="sample-list-item"
                        >
                           <template v-if="model.key === 'organisms'">
                              <span class="va-text-secondary" style="font-style: italic">{{
                                 row.scientific_name
                              }}</span>
                              <RouterLink
                                 :to="{ name: 'item', params: { model: 'organisms', id: row.taxid } }"
                                 target="_blank"
                                 class="va-link ml-2"
                              >
                                 {{ row.taxid }}
                           ...
                           <template v-else-if="model.key === 'biosamples' || model.key === 'assemblies'">
                              ...
                              <RouterLink
                                 :to="{ name: 'item', params: { model: model.key, id: row.accession } }"
                                 target="_blank"
                                 class="va-link ml-2"
                              >
                                 {{ row.accession }}
                           ...
                           <template v-else-if="model.key === 'annotations'">
                              ...
                              <RouterLink
                                 :to="{ name: 'item', params: { model: 'annotations', id: row.name } }"
                                 target="_blank"
                                 class="va-link ml-2"
                              >
                                 {{ row.name }}
                           ...
                           <template v-else-if="model.key === 'local_samples'">
                              ...
                              <RouterLink
                                 :to="{ name: 'item', params: { model: 'local_samples', id: row.local_id } }"
                                 target="_blank"
                                 class="va-link ml-2"
                              >
                                 {{ row.local_id }}
                           ...
                           <template v-else-if="model.key === 'reads'">
                              <span class="va-text-secondary" style="font-style: italic">{{
                                 row.scientific_name || row.metadata.scientific_name
                              }}</span>
                              <RouterLink
                                 :to="{ name: 'item', params: { model: 'reads', id: row.run_accession } }"
                                 target="_blank"
                                 class="va-link ml-2"
                              >
