    ...
    <va-card-title>
      <div class="row align-center justify-space-between">
        ...
        <div style="padding: 0" class="flex">
          <va-button :disabled="contributors.length <= step" preset="secondary" icon="chevron_right"
            @click="showNext" />
        </div>
      </div>
    </va-card-title>
    <va-card-content>
      <div v-for="(contributor, idx) in visibleList" :key="idx" class="mb-3">
        <va-progress-bar :model-value="getPercent(contributor.contributions)" color="primary">
          {{ contributor.contributions }} {{ chart.model }}
        </va-progress-bar>
...
import { Contributor, InfoBlock } from '../../data/types'
