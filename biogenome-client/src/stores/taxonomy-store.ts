...
import { ref } from 'vue'
import { flattenedTreeToNested } from '../composable/flattenedTreeToNested'
import { taxonNodeToStatsTuples } from '../composable/taxonNodeStats'
...
export const useTaxonomyStore = defineStore('taxonomy', {
   ...
   actions: {
      ...
      async setCurrentTaxon(taxon: TaxonNode) {
         this.currentTaxon = { ...taxon }
         await this.getAncestors(taxon.taxid)
      },
      ...
      async getStats(taxid: string) {
         const { data } = await TaxonService.getTaxon(taxid)
         this.stats = taxonNodeToStatsTuples(data as Record<string, unknown>)
      },
      ...
      async getTree() {
         ...
         try {
            ...
            if (data?.fields && Array.isArray(data?.rows)) {
               const { tree, rootNode } = flattenedTreeToNested(data)
               this.treeData = tree
               if (!this.rootNode) {
                  this.rootNode = {
                     name: rootNode.name,
