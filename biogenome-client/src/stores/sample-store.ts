   ...
   actions: {
      ...
      mapFormToEBIPayload(checklistFields: { name: string; unit?: string }[]) {
         ...
         return {
            ...
            ...Object.fromEntries(
               Object.entries(this.characterics)
                  .filter(([k, v]) => v)
                  .map(([k, v]) => [
                     k,
                     convertToSampleCharacteristic(mapValue(v), undefined, getUnit(k, checklistFields)),
                  ]),
            ),
         }
      },
      async submitSample(checklistFields: { name: string; unit?: string }[]) {
         ...
         try {
            this.loading = true
            const { data } = await EBIService.submitBiosample({
               name: this.sampleIdentifier,
               taxid: this.taxid,
               release: this.yesterdayISO(),
               characteristics,
            })
            ...
            if (responseData && Array.isArray(responseData)) {
               this.validationErrors = responseData
                  .map((err: { dataPath: string; errors: string[] }) => `${err.dataPath}: ${err.errors.join('; ')}`)
                  .flat()
            }
