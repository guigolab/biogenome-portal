...
def full_text_search(params,model):
    ...
    if query_status:
        organisms = model.objects(query_tax & query_status & query).only('common_name','taxid','organism', 'trackingSystem')
    else:
        organisms = model.objects(query_tax & query).only('common_name','taxid','organism', 'trackingSystem')
...
def default_query_params(params,model):
    ...
    query_status = query_by_status(resolved_params['status']) if resolved_params['status'] else None
    if query_status:
        organisms = model.objects(query_tax & query_status).only('common_name','taxid','organism', 'trackingSystem')
    else:
        organisms = model.objects(query_tax).only('common_name','taxid','organism', 'trackingSystem')
    if resolved_params['sortColumn']:
        sort = '-'+resolved_params['sortColumn'] if resolved_params['sortOrder'] == 'true' else resolved_params['sortColumn']
        organisms = organisms.order_by(sort)
    json_resp['total'] = organisms.count()
