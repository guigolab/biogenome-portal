...
from helpers.rest_catalog_sync import cascade_delete_assembly, sync_species_after_catalog_change
...
_CHR_ALIASES_HEADER = (
    "Assigned-Molecule\tSequence-Name\tUCSC-style-name\tRefSeq-Accn\taccession_version\n"
)


def _chr_aliases_tsv_cell(meta: dict, *keys: str) -> str:
    """First non-empty, non-placeholder metadata value among alternate column names."""
    for key in keys:
        raw = meta.get(key)
        if raw is not None and not assembly_helper.is_ncbi_report_na_cell(raw):
            return str(raw).strip()
    return ""


def get_related_chromosomes(accession, args):
    ass = get_or_404(Assembly, f"Assembly {accession} not found", accession=accession)
...
def create_assembly_from_accession(accession):
    ...
    assembly_helper.save_chromosomes_from_assembly_report(assembly_obj)
    ...
    assembly_obj.save()
    # Match job finalize: copy Organism lineage onto the assembly so TaxonNode roll-ups
    # (aggregations over catalog taxon_lineage) include this row for ancestors.
    data.update_lineage(assembly_obj, organism_obj, skip_sync=True)
    sync_species_after_catalog_change(
        str(assembly_obj.taxid), organism_obj.taxon_lineage
    )

    return accession

def get_blobtoolkit_id(accession):
    return genomehubs_client.fetch_blobtoolkit_first_name(accession)
...
def delete_assembly(accession):
    ...
    cascade_delete_assembly(assembly_obj)
    return accession

def get_related_annotations(accession, args):
    get_or_404(Assembly, f"Assembly {accession} not found", accession=accession)
...
def get_chr_aliases_file(accession):
    ...
    def stream_aliases():
        yield _CHR_ALIASES_HEADER.encode("utf-8")
        qs = chromosomes.no_cache().only("metadata", "accession_version").order_by(
            "accession_version"
        )
        for chromosome in qs:
            meta = chromosome.metadata or {}
            assigned = _chr_aliases_tsv_cell(meta, "Assigned-Molecule", "assigned_molecule")
            seq_name = _chr_aliases_tsv_cell(meta, "Sequence-Name", "sequence_name")
            ucsc = _chr_aliases_tsv_cell(meta, "UCSC-style-name", "ucsc_style_name")
            refseq = _chr_aliases_tsv_cell(meta, "RefSeq-Accn", "refseq_accn")
            av = chromosome.accession_version or ""
            yield f"{assigned}\t{seq_name}\t{ucsc}\t{refseq}\t{av}\n".encode("utf-8")
