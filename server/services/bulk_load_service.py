...
def load_data(model,data):
    ...
    if model == 'assembly':
        for ass in data:
            ass_obj = assembly_service.create_assembly_from_ncbi_data(ass, ass['biosample_accession'])
            data_helper.create_data_from_assembly(ass_obj,ass)
    elif model == 'biosamples':
        for project in data.keys():
            for sample in data[project]:
                biosample_obj = biosample_service.create_biosample_from_ebi_data(sample)
