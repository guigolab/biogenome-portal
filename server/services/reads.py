...
def create_read_runs_from_ena_accession(accession: str) -> str:
    ...
    for rr in parsed_runs:
        ...
        if existing:
            ...
            data_helper.update_lineage(existing, organism_obj, skip_sync=True)
        else:
            rr.save()
    ...
    for tid in sorted(taxids_to_refresh):
        org = organism_cache.get(tid)
        lineage = list(org.taxon_lineage) if org and getattr(org, "taxon_lineage", None) else None
        sync_species_after_catalog_change(tid, lineage)

    return accession


def delete_read_run(run_accession: str) -> str:
    run = get_or_404(ReadRun, f"Read run {run_accession} not found!", run_accession=run_accession)
    taxid = run.taxid
    taxon_lineage = run.taxon_lineage
    run.delete()
